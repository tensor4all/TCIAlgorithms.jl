...
@testset "mul" begin
    ...
    @testset "batchevaluate" begin
        ...
        for x in 1:2, y in 1:3
            @test batchmul[1, 1, TCIA._lineari(ab.sitedims[3], [x, y]), 1] ≈
                ab([[1, 2], [1, 1], [x, y], [1, 1]])
        end
    end
    ...
    @testset "containermul" begin
        ...
        for i in 1:4
            ...
            projector_a = Projector(
                [TCIA._multii(sitedimsa[1], i), [0, 0], [0, 0], [0, 0]], sitedimsa
            )
            push!(pa, project(makeprojectable(a), projector_a))

            projector_b = Projector(
                [reverse(TCIA._multii(sitedimsb[1], i)), [0, 0], [0, 0], [0, 0]], sitedimsb
            )
